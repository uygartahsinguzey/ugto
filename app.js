(() => {
  ...
  function getMikiMood() {
    if (state.miki.sleep < 28) return { label: "Biraz uykulu", tone: "sleepy" };
    if (state.miki.satiety < 28) return { label: "Biraz acıktı", tone: "hungry" };
    if (state.miki.happiness >= 82) return { label: "Enerjisi yüksek", tone: "excited" };
    if (state.miki.love >= 82) return { label: "Sana çok bağlı", tone: "loving" };
    return { label: "Modu iyi", tone: "calm" };
  }
  ...
  function renderMiki() {
    ...
    const cat = $("#pixelCat");
    cat.className = `pixel-cat costume-${costume} ${mikiMotion}`;
    cat.setAttribute("aria-label", `${state.profile.mikiName}, ${costumeName} görünümüyle özgün dijital kedi`);
  ...
  function interactWithMiki(action) {
    const messages = {
      ...
      feed: `${state.profile.mikiName} atıştırmalığını keyifle bitirdi.`,
      play: `${state.profile.mikiName} topun peşinden fırladı!`,
      rest: `${state.profile.mikiName} sakin bir köşe bulup kıvrıldı...`
    };
    ...
    const motionMap = { pet: "motion-love", feed: "motion-eat", play: "motion-play", rest: "motion-sleep" };
    save(); renderHeader(); renderToday();
    playMikiMotion(motionMap[action], action === "rest" ? 2600 : 1900);
  }
  ...
  function renderRoom() {
    ...
    roomCat.className = `room-cat room-costume-${costume}`;
  ...
  function updateTimerDisplay() {
    ...
    document.title = timer.running ? `${pad(minutes)}:${pad(seconds)} · ${state.profile.appName}` : `${state.profile.appName} V15.1`;
  }
