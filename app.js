(() => {
  ...
  const COSTUMES = [
    { id: "classic", name: "Klasik Miki", icon: "M", unlockAt: 0, description: "Miki'nin özgün başlangıç görünümü." },
    { id: "chef", name: "Şef Miki", icon: "Ş", unlockAt: 3, description: "3 odak seansında açılır." },
    { id: "student", name: "Öğrenci Miki", icon: "Ö", unlockAt: 8, description: "8 odak seansında açılır." },
    { id: "pirate", name: "Korsan Miki", icon: "K", unlockAt: 15, description: "15 odak seansında açılır." },
    { id: "wizard", name: "Büyücü Miki", icon: "B", unlockAt: 25, description: "25 odak seansında açılır." },
    { id: "astronaut", name: "Astronot Miki", icon: "A", unlockAt: 40, description: "40 odak seansında açılır." }
  ];
  ...
  function syncUnlocks(showMessage = false) {
    ...
    COSTUMES.forEach(item => {
      if (total >= item.unlockAt && !state.miki.ownedCostumes.includes(item.id)) {
        state.miki.ownedCostumes.push(item.id);
        if (item.id !== "classic") newlyUnlocked.push(item.name);
      }
    });
  ...
  function renderHeader() {
    ...
    document.title = `${state.profile.appName} V16`;
  }
  ...
  function nextUnlockHint() {
    const total = totalPomodoros();
    const next = [...COSTUMES.filter(item => item.unlockAt > total), ...ROOM_ITEMS.filter(item => item.unlockAt > total)]
  ...
  function renderMiki() {
    ...
    const costume = COSTUMES.find(item => item.id === state.miki.equippedCostume) || COSTUMES[0];
    $("#equippedCostumeTag").textContent = costume.name.replace(" Miki", "");

    const cat = $("#pixelCat");
    cat.className = `pixel-cat costume-${costume.id} ${mikiMotion}`;
    cat.setAttribute("aria-label", `${state.profile.mikiName}, ${costume.name} görünümüyle özgün dijital kedi`);
    ...
    const total = totalPomodoros();
    $("#wardrobe").innerHTML = COSTUMES.map(item => {
      const owned = state.miki.ownedCostumes.includes(item.id) || total >= item.unlockAt;
      const equipped = state.miki.equippedCostume === item.id;
      return `<button class="wardrobe-card ${owned ? "" : "locked"} ${equipped ? "active" : ""}" data-costume="${item.id}" ${owned ? "" : "disabled"}>
        <span class="wardrobe-mark">${item.icon}</span><strong>${item.name}</strong><small>${owned ? equipped ? "Seçili" : "Seç" : `${item.unlockAt} odak seansında açılır`}</small>
      </button>`;
    }).join("");

    $$(".wardrobe-card:not(.locked)").forEach(button => button.addEventListener("click", () => {
      state.miki.equippedCostume = button.dataset.costume;
      if (!state.miki.ownedCostumes.includes(button.dataset.costume)) state.miki.ownedCostumes.push(button.dataset.costume);
      save();
      $("#mikiSpeech").textContent = `${state.profile.mikiName} yeni görünümünü sevdi.`;
      playMikiMotion("motion-wave", 1900);
      renderRoom();
    }));
  }
  ...
  function interactWithMiki(action) {
    const messages = {
      pet: `${state.profile.mikiName} gözlerini kısıp mırıldandı.`,
      play: `${state.profile.mikiName} topun peşinden fırladı.`,
      wave: `${state.profile.mikiName} patisini kaldırıp seni selamladı.`,
      rest: `${state.profile.mikiName} sakin bir köşeye kıvrıldı.`
    };
    const motionMap = { pet: "motion-love", play: "motion-play", wave: "motion-wave", rest: "motion-sleep" };
  ...
  function startMikiAmbientAnimations() {
    window.setInterval(() => {
      ...
      const motions = ["motion-look", "motion-stretch", "motion-wave", "motion-idle"];
      const selected = motions[Math.floor(Math.random() * motions.length)];
      if (selected !== "motion-idle") playMikiMotion(selected, 1800);
    }, 6800);
  }
  ...
  function renderRoom() {
    ...
    roomCat.className = `room-cat room-costume-${state.miki.equippedCostume}`;
    roomCat.setAttribute("aria-label", `${state.profile.mikiName} odada dolaşıyor`);
  }
  ...
  function updateTimerDisplay() {
    ...
    document.title = timer.running ? `${pad(minutes)}:${pad(seconds)} · ${state.profile.appName}` : `${state.profile.appName} V16`;
  }
  ...
  function completeTimer() {
    ...
    if (timer.mode === "focus") {
      ...
      toast("Odak seansı tamamlandı. Miki ile bağın güçlendi.");
    } else {
      setTimerMode("focus");
